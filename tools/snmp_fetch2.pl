  ...
  #print "Switch: $switch \n";
    foreach my $port (keys %{ $port_data{$switch} }) {
      #print "Port: $port \n";
        ...
        $sql2 = "SELECT ports.id,switches.name, switches.id AS swid FROM ports JOIN switches WHERE ports.switch_id = '$switch' AND ports.ifName = '$if_name' AND switches.id = ports.switch_id";
        print $sql2 ."\n";
...
$current_bytes_out = 0;
    ...
    $dbh->do("UPDATE `ports` SET `current_in` =  '$current_bytes_in',`current_out` =  '$current_bytes_out',`updated` =  '$epoc' WHERE  `id` ='$id'");
        ...
        if($sth2->rows == 0) {
          #ADD
          $epoc = time();
          $dbh->do("INSERT INTO `ports` (switch_id,ifName,ifIndex) VALUES ('$switch', '$if_name','$port')");
        }
