      ...
      echo "<tr>\n";
        ...
        foreach ($block[$y] as $thing) {
          ...
          elseif(@$switches[$x][$y] != NULL) {
            ...
            $sql = "SELECT `name`,`model`,`distro_id`,`distro_port`,`alive` FROM `switches` WHERE `id` = $switch_id";
            $result = mysqli_query($con,$sql);
            while($row = mysqli_fetch_array($result))
            {
              $switch_name = $row["name"];
              ?>
              <td id="<?php echo $x.".".$y; ?>" class="<?php if($row["alive"] == 1) {echo "green";}else { echo "red";} ?>" onmouseover="tooltip.show('<strong><?php echo $switch_name; ?></strong>');" onmouseout="tooltip.hide();"></td>
              <?php
            }
            $x++;
}
